...
void RunLocal(TString inputFileName = "AliAOD.root", Int_t nEvents = 1e6, Bool_t isESD = kFALSE, Bool_t isMC = kFALSE, Int_t debugLevel = 0)
{
  ...
  AliAnalysisManager *mgr = new AliAnalysisManager("SimpleMuonTask");
 ...
 // Enable MC event handler for ESDs
  ...
  gROOT->LoadMacro("AliDoubleJpsi.cxx+g");
  gROOT->LoadMacro("AddDoubleJpsi.C");
  AliDoubleJpsi* task = AddDoubleJpsi(kTRUE,"JpsiTask.root");
  if (!task) {
    Error("runAnalysis","AliDoubleJpsi not created!");
    return;
  }
