...
void RunPoD(
  ...
  gProof->Load("AliDoubleJpsi.cxx+");  // DON'T use double '+' when running multiple times: it uselessly recompiles everything!
  gROOT->LoadMacro("AddDoubleJpsi.C");

  TString outputFileName;
  outputFileName.Form("AnalysisResults_run%09d.root", runNumber);
  AliDoubleJpsi *simplePtTask = AddDoubleJpsi(usePhysicsSelection, outputFileName);
