...
void RunPoDCharge(
  ...
  gProof->Load("AliAnalysisMuonsCharge.cxx+g");  // DON'T use double '+' when running multiple times: it uselessly recompiles everything!
  gROOT->LoadMacro("AddTaskMuonsCharge.C");

  TString outputFileName;
  outputFileName.Form("AnalysisResultsCharge_run%09d.root", runNumber);
  AliAnalysisMuonsCharge *simplePtTask = AddTaskMuonsCharge(usePhysicsSelection, outputFileName);
